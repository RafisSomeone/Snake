...
def plot_data(data, xaxis='Epoch', value="AverageEpRet", condition="Condition1", smooth=10, **kwargs):
    ...
    sns.lineplot(data=datum, x=range(len(data)), y='ok', ci='sd')
    """
...
def make_plots(all_logdirs, legend=None, xaxis=None, values=None, count=False,  
    ...
    results =[]
    s = 1
    with open('Reward3.csv', newline='') as csvfile:
        spamreader = csv.reader(csvfile, delimiter=' ', quotechar='|')
        for row in spamreader:
            results.append(float(row[0]))
    data = pd.DataFrame(results, columns =['ok'])
